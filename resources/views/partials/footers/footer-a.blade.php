<div class="md:flex md:flex-row md:items-center md:justify-between">
  ...
  <nav>
    @if (has_nav_menu('primary_navigation'))
      {!! wp_nav_menu(['theme_location' => 'footer_navigation', 'menu_class' => 'nav', 'container' => '']) !!}
    @endif
  </nav>
</div>
<div class="md:flex md:flex-row md:items-center md:justify-between mt-5 pt-5 border-t border-solid border-gray-100">
  <div>
    @if( App::siteSocialLinks() )
      @foreach( App::siteSocialLinks() as $link )
        <a class="inline-flex items-center justify-center w-icon h-icon bg-primary-1 rounded" href="{{ $link['url'] }}" aria-labelledby="{{ strtolower($link['title']) }}">
  ...
  <div>
    <p class="text-xs">
      ...
      <a href="{{ get_permalink(68) }}">ADA Compliance</a> &#124;
      <a href="{{ get_permalink(70) }}">Privacy Policy</a>
      <span class="hidden md:inline-block">&#124;</span>
      <span class="block md:inline-block">Website By <a href="https://www.bigrigxpress.com/xpress-website-development/">Big Rig Media LLC</a> &reg;</span>
    </p>
  </div>
</div>
